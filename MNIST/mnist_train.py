...
def main():
  ...
  # for these loss functions we should be careful that the learning rate should be modified accordingly, since the sum now has gradients 100 times larger than the mean (100 points in the mini-batch). Therefore, we should reduce the learning rate with that factor
  #cross_entropy = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=y_,logits=tf.matmul(x,W) + b))
  #cross_entropy = tf.norm(tf.nn.softmax_cross_entropy_with_logits(labels=y_,logits=tf.matmul(x,W) + b),1)
  #cross_entropy = tf.norm(tf.nn.softmax_cross_entropy_with_logits(labels=y_,logits=tf.matmul(x,W) + b),2)
